...
const Page: React.FC = () => {
  ...
  const handler = () => {
    router.push(`/read/technologyNews/0`);
  };
      ...
      >
        <div className="absolute right-0 top-0 p-6 flex items-center group">
          ...
          <div className="absolute z-30 right-6 top-6 py-4 px-6 flex flex-col space-y-1 rounded-2xl opacity-0 group-hover:opacity-100 w-0 group-hover:w-80 animation-smooth-700 text-sm bg-white">
            <span>
              {new Date(
                newsData.technologyNews[0].publishedAt
              ).toLocaleDateString("en-GB")}
            </span>
            <span>{newsData.technologyNews[0].author}</span>
            <p className="h-20 overflow-y-hidden">
              {newsData.technologyNews[0].description}
            </p>
          </div>
        </div>
        <div className="w-full h-[400px] rounded-3xl overflow-hidden group">
          <Image
            ...
            priority
            unoptimized
            src={newsData.technologyNews[0].urlToImage}
            alt="news illustration"
            width={1000}
            height={1000}
          />
        ...
        <h2 className="absolute left-0 bottom-0 text-white md:max-w-[60%] text-2xl md:text-3xl lg:text-4xl leading-7 m-5 lg:m-10">
          {newsData.technologyNews[0].title}
        </h2>
      </section>
      <div className="w-[80vw] md:flex md:space-x-4 space-y-4 md:space-y-0 mb-4">
        <NewsItem {...newsData.technologyNews[1]} id={1} type={"technology"} />
        <NewsItem {...newsData.technologyNews[2]} id={2} type={"technology"} />
        <NewsItem {...newsData.technologyNews[3]} id={3} type={"technology"} />
      </div>
      <div className="w-[80vw] md:flex md:space-x-4 space-y-4 md:space-y-0 mb-4">
        <NewsItem {...newsData.technologyNews[4]} id={4} type={"technology"} />
        <NewsItem {...newsData.technologyNews[5]} id={5} type={"technology"} />
        <NewsItem {...newsData.technologyNews[6]} id={6} type={"technology"} />
      </div>
      <div className="w-[80vw] md:flex md:space-x-4 space-y-4 md:space-y-0 mb-4">
        <NewsItem {...newsData.technologyNews[7]} id={7} type={"technology"} />
        <NewsItem {...newsData.technologyNews[8]} id={8} type={"technology"} />
        <NewsItem {...newsData.technologyNews[9]} id={9} type={"technology"} />
      </div>
    </main>
  );
};
