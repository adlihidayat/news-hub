...
const Page: React.FC = () => {
  ...
  const handler = () => {
    router.push(`/read/financeNews/0`);
  };
      ...
      >
        <div className="absolute right-0 top-0 p-6 flex items-center group">
          ...
          <div className="absolute z-30 right-6 top-6 py-4 px-6 flex flex-col space-y-1 rounded-2xl opacity-0 group-hover:opacity-100 w-0 group-hover:w-80 animation-smooth-700 text-sm bg-white">
            <span>
              {new Date(newsData.financeNews[0].publishedAt).toLocaleDateString(
                "en-GB"
              )}
            </span>
            <span>{newsData.financeNews[0].author}</span>
            <p className="h-20 overflow-y-hidden">
              {newsData.financeNews[0].description}
            </p>
          </div>
        </div>
        <div className="w-full h-[400px] rounded-3xl overflow-hidden group">
          <Image
            ...
            priority
            unoptimized
            src={newsData.financeNews[0].urlToImage}
            alt="news illustration"
            width={1000}
            height={1000}
          />
        ...
        <h2 className="absolute left-0 bottom-0 text-white md:max-w-[60%] text-2xl md:text-3xl lg:text-4xl leading-7 m-5 lg:m-10">
          {newsData.financeNews[0].title}
        </h2>
      </section>
      <div className="w-[80vw] md:flex md:space-x-4 space-y-4 md:space-y-0 mb-4">
        <NewsItem {...newsData.financeNews[1]} id={1} type={"finance"} />
        <NewsItem {...newsData.financeNews[2]} id={2} type={"finance"} />
        <NewsItem {...newsData.financeNews[3]} id={3} type={"finance"} />
      </div>
      <div className="w-[80vw] md:flex md:space-x-4 space-y-4 md:space-y-0 mb-4">
        <NewsItem {...newsData.financeNews[4]} id={4} type={"finance"} />
        <NewsItem {...newsData.financeNews[5]} id={5} type={"finance"} />
        <NewsItem {...newsData.financeNews[6]} id={6} type={"finance"} />
      </div>
      <div className="w-[80vw] md:flex md:space-x-4 space-y-4 md:space-y-0 mb-4">
        <NewsItem {...newsData.financeNews[7]} id={7} type={"finance"} />
        <NewsItem {...newsData.financeNews[8]} id={8} type={"finance"} />
        <NewsItem {...newsData.financeNews[9]} id={9} type={"finance"} />
      </div>
    </main>
  );
};
